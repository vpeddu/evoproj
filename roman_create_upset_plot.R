...
tmp %>% 
  mutate(Human = TRUE) %>%
  mutate(insertion_x = str_replace(Human, pattern = '\\[', replacement = ''),
         insertion_x = str_replace(insertion_x, pattern = '\\]', replacement = ''),
         #insertion_x = str_replace_all(insertion_x = ' ', replacement = ''),
         insertion_x = as.list(as.vector(insertion_x))) %>% 
  unnest(insertion_x) %>% 
...
tmp_merge_df %>% 
  ...
  summarize(species = unique(list(species)), 
            ...
            mean_exp = mean(log1p(value.y)), sd_exp = sd(log1p(value.y))) -> x
