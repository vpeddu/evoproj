...
with open('lnc_TEbag_DB.pkl', 'rb') as f:
    TE_DB = pickle.load(f)

#quant_file = pd.read_csv (sys.argv[2], sep='\t', header=0)
quant_file = pd.read_csv('ctrl_panc_covid_tx_counts.tsv', sep = '\t', header = 0)
#from IPython import embed; embed()
# removing non-TE rows
TE_quants = quant_file[quant_file["insertion"].str.contains("ENST")==True]
...
# set TE TPM cutoff to 5
#TE_quants = TE_quants[TE_quants["TPM"] > 5
#TE_quants = TE_quants.reset_index(drop = True)
...
TE_insertions['insertion'] = TE_insertions.apply(lambda row: check_if_in_species(row), axis=1)
